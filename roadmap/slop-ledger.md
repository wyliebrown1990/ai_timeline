...
| LEDGER-004 | 2026-07-02 | Plan Review: Blog SSR & Indexability | 15 Schema/infra risk + 16 Process (IaC drift) | CloudFront distribution `E23Z9QNRPDI3HW` — not declared in any IaC (`infra/template.yaml` / `infra/edge-og-tags/template.yaml`) | P3 | OPEN | The production CloudFront distribution is managed out-of-band — no `AWS::CloudFront::Distribution` resource exists; behaviors/origins/edge-associations are all live-console/CLI state. Sprint SSR-2 has to add the `/blog*` behavior via `aws cloudfront update-distribution` because SAM can't manage what it doesn't own. Acceptable stopgap (scripted, not click-ops), but the distribution should eventually be imported into SAM (or a dedicated CloudFront stack) so behavior changes are reviewable and reproducible. NOT in scope for the SSR initiative — logged for a future infra-hardening pass. Blast radius: any future CDN behavior/cache/edge change is manual and undocumented until imported. |
