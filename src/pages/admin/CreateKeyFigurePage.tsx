...
export function CreateKeyFigurePage() {
  const navigate = useNavigate();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isGeneratingVariants, setIsGeneratingVariants] = useState(false);
  const [idPreview, setIdPreview] = useState('');

  const [formData, setFormData] = useState<CreateKeyFigureDto>({
    canonicalName: '',
    aliases: [],
    shortBio: '',
    fullBio: '',
  ...
  // Generate ID preview when canonical name changes
  const handleNameChange = async (name: string) => {
    setFormData({ ...formData, canonicalName: name });
    if (name.trim().length >= 2) {
      try {
        const result = await keyFiguresApi.generateVariants(name);
        setIdPreview(result.id);
      } catch {
        setIdPreview('');
      }
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newFigure = await keyFiguresApi.create(formData);
      toast.success(`Created ${newFigure.canonicalName}`);
      navigate('/admin/key-figures');
    } catch (error: unknown) {
      const apiError = error as { message?: string };
      toast.error(apiError.message || 'Failed to create key figure');
      console.error(error);
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Add Key Figure</h1>
          <p className="mt-1 text-sm text-gray-500">
            Add a notable person in AI history
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div>
            ...
            {idPreview && (
              <p className="mt-1 text-sm text-gray-500">
                ID: <code className="bg-gray-100 px-1.5 py-0.5 rounded">{idPreview}</code>
              </p>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Brief one-line bio (e.g., 'British-Canadian cognitive psychologist and computer scientist, known as the godfather of deep learning')"
              rows={2}
              maxLength={500}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white"
              required
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Key contributions (e.g., 'Pioneering work on backpropagation, Boltzmann machines, and deep neural networks')"
              rows={2}
              maxLength={500}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white"
              required
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Extended biography with career history, education, and achievements..."
              rows={4}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white"
            />
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Add alias (e.g., Geoff Hinton)"
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Add organization (e.g., University of Toronto)"
              className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white"
            />
            <button
              type="button"
        ...
        <div className="flex gap-3">
          ...
          >
            {isSubmitting ? 'Creating...' : 'Create Key Figure'}
          </button>
          <button
            type="button"
            onClick={() => navigate('/admin/key-figures')}
...
export default CreateKeyFigurePage;
