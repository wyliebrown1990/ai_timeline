 ...
 * Run with: npx tsx scripts/add-bhoi-content-api.ts
 */
...
async function getCredentials(): Promise<{ username: string; password: string }> {
  ...
  const username = execSync(
    'aws ssm get-parameter --name "/ai-timeline/prod/admin-username" --with-decryption --query "Parameter.Value" --output text',
    { encoding: 'utf-8' }
  ).trim();
...
const persons = [
  {
    ...
    focusAreas: ['behavioral psychology', 'learning theory', 'educational psychology'],
    aliases: ['Edward L. Thorndike', 'E.L. Thorndike'],
  },
  {
    ...
    focusAreas: ['physiology', 'classical conditioning', 'digestion'],
    aliases: ['I.P. Pavlov', 'Pavlov'],
  },
  {
    ...
    focusAreas: ['behaviorism', 'operant conditioning', 'behavior analysis'],
    aliases: ['Burrhus Frederic Skinner', 'B. F. Skinner'],
  },
  {
    ...
    focusAreas: ['evolutionary neuroscience', 'limbic system', 'brain evolution'],
    aliases: ['Paul D. MacLean'],
  },
  {
    ...
    focusAreas: ['neuropsychology', 'learning', 'synaptic plasticity'],
    aliases: ['D.O. Hebb', 'Donald O. Hebb'],
  },
  {
    ...
    focusAreas: ['dopamine', 'reward learning', 'decision-making', 'neuroeconomics'],
    aliases: [],
  },
  {
    ...
    focusAreas: ['computational neuroscience', 'predictive processing', 'free energy principle', 'brain imaging'],
    aliases: ['K.J. Friston', 'Karl J. Friston'],
  },
  {
    ...
    focusAreas: ['amygdala', 'fear', 'emotion', 'memory'],
    aliases: ['Joseph E. LeDoux', 'Joe LeDoux'],
  },
  {
    ...
    focusAreas: ['computational neuroscience', 'dopamine', 'decision-making', 'neuroeconomics'],
    aliases: ['P. Read Montague'],
  },
  {
    ...
    focusAreas: ['computational neuroscience', 'vision', 'cerebellum'],
    aliases: ['David C. Marr'],
  },
  {
    ...
    focusAreas: ['reinforcement learning', 'temporal difference learning', 'AI'],
    aliases: ['Richard S. Sutton', 'Rich Sutton'],
  },
  {
    ...
    focusAreas: ['reinforcement learning', 'actor-critic', 'machine learning'],
    aliases: ['Andrew G. Barto', 'Andy Barto'],
  },
  {
    ...
    focusAreas: ['hierarchical temporal memory', 'neocortex', 'probabilistic models', 'robotics'],
    aliases: [],
  },
];
...
const milestones = [
  {
    ...
    category: 'research',
    significance: 4,
    tags: ['neuroscience', 'psychology', 'learning', 'classical-conditioning'],
  },
  {
    ...
    category: 'research',
    significance: 3,
    tags: ['neuroscience', 'psychology', 'reinforcement-learning', 'behaviorism'],
  },
  {
    ...
    category: 'research',
    significance: 4,
    tags: ['neuroscience', 'neural-networks', 'learning', 'synaptic-plasticity'],
  },
  {
    ...
    category: 'research',
    significance: 3,
    tags: ['neuroscience', 'brain-evolution', 'limbic-system'],
  },
  {
    ...
    category: 'research',
    significance: 4,
    tags: ['reinforcement-learning', 'machine-learning', 'algorithms'],
  },
  {
    ...
    category: 'research',
    significance: 4,
    tags: ['neuroscience', 'dopamine', 'reinforcement-learning', 'prediction-error'],
  },
  {
    ...
    category: 'research',
    significance: 3,
    tags: ['neuroscience', 'predictive-processing', 'free-energy', 'theory'],
  },
];
...
async function createGlossaryTerm(token: string, term: typeof glossaryTerms[0]): Promise<boolean> {
  const response = await fetch(`${API_BASE}/api/admin/glossary`, {
    method: 'POST',
    headers: {
  ...
  if (response.status === 400) {
    ...
    throw new Error(`Failed to create term ${term.term}: ${response.status} ${text}`);
  }
...
async function createPerson(token: string, person: typeof persons[0]): Promise<boolean> {
  const response = await fetch(`${API_BASE}/api/admin/persons`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Failed to create person ${person.canonicalName}: ${response.status} ${text}`);
  }

  return true;
}

async function createMilestone(token: string, milestone: typeof milestones[0]): Promise<boolean> {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(`Failed to create milestone ${milestone.title}: ${response.status} ${text}`);
  }
...
async function main() {
  ...
  for (const term of glossaryTerms) {
    try {
      const created = await createGlossaryTerm(token, term);
      if (created) {
  ...
  for (const person of persons) {
    try {
      const created = await createPerson(token, person);
      if (created) {
  ...
  for (const milestone of milestones) {
    try {
      const created = await createMilestone(token, milestone);
      if (created) {
  ...
  console.log('\nNote: Run subject classification backfill to assign subjects to new content.');
}
